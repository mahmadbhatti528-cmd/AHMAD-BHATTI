/**
   * Create By AHMADISBRAND OFFICIAL
   * Contact Me on 447384369153
*/
...
const initializeBot = async () => {
    ...
    console.log(chalk.green('   𝐌𝐑𝐈𝐒𝐁𝐑𝐀𝐍𝐃 𝐩𝐚𝐢𝐫𝐢𝐧𝐠 𝐬𝐲𝐬𝐭𝐞𝐦       '));
    console.log(chalk.yellow('═══════════════════════════════════════════════\n'));
...
function launchBot() {
    console.clear();
    console.log(chalk.green('🚀 Starting MR IS BRAND system...\n'));
    ...
    if (fs.existsSync(botPath)) {
        try {
            ...
            require('./bot');
            telegramLoaded = true;
            console.log(chalk.green('MRISBRAND bot loaded successfully!'));
        } catch (error) {
    ...
    console.log(chalk.bold.white('MRISBRAND BOT INITIALIZATION SUMMARY          '));
    console.log(chalk.cyan('═══════════════════════════════════════════════'));
    console.log(telegramLoaded ? chalk.green('✅ ᴍʀɪsʙʀᴀɴᴅ тɛℓɛɢяαм вσт: Active') : chalk.red('❌ ᴍʀɪsʙʀᴀɴᴅ тɛℓɛɢяαм вσт : Inactive'));
    ...
    if (!telegramLoaded && !whatsappLoaded) {
        ...
        console.log(chalk.green('✅ MRISBRAND system is ready and running!\n'));
    }
